...
void
MainConsole::setText(const QString &text)
{
  QMutexLocker lock(&mutex_);
  Base::setText(text_ = text);
  updateFormat();
}
...
void
MainConsole::updateFormat()
{
  //QTextCursor tc = textCursor();
  //tc.select(QTextCursor::Document);
  //QTextCharFormat fmt;
  //fmt.setTextOutline(QPen(Qt::white, 0.1)); // QPen(brush, width)
...
MainConsole::sendMessage(const QString &t)
{
  ...
  if (isAreaFull()) {
    ...
    emit asyncSetText(text_.append(
