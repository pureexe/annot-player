...
AnnotationStandardModel::AnnotationStandardModel(SignalHub *hub, QObject *parent)
  : Base(0, HD_Count, parent), hub_(hub)
{ resetHeader(); }

Qt::ItemFlags
AnnotationStandardModel::flags(const QModelIndex &index) const
{ return Base::flags(index) & ~Qt::ItemIsEditable; } // read-only

void
AnnotationStandardModel::setHeaderData(QAbstractItemModel *model)
{
  Q_ASSERT(model);
...
void
AnnotationStandardModel::addAnnotation(const Annotation &a)
{
...
int
AnnotationStandardModel::rowById(qint64 id) const
{
...
qint64
AnnotationStandardModel::indexId(const QModelIndex &input)
{
  if (!input.isValid())
    return 0;
  int row = input.row();
...
qint64
AnnotationStandardModel::indexUserId(const QModelIndex &input)
{
  if (!input.isValid())
    return 0;
  int row = input.row();
...
QString
AnnotationStandardModel::indexUserAlias(const QModelIndex &input)
{
  if (!input.isValid())
    return QString();
...
QString
AnnotationStandardModel::indexText(const QModelIndex &input)
{
  if (!input.isValid())
    return QString();
...
QString
AnnotationStandardModel::languageToString(int lang)
{
  switch(lang) {
...
QStringList
AnnotationStandardModel::annotationFlagsToStringList(int flags)
{
  QStringList ret;
...
QString
AnnotationStandardModel::annotationStatusToString(int status)
{
  switch (status) {
...
void
AnnotationStandardModel::updateAnnotationTextWithId(const QString &text, qint64 id)
{
  int row = rowById(id);
  if (row >= 0) {
...
void
AnnotationStandardModel::updateAnnotationUserIdWithId(qint64 userId, qint64 id)
{
  int row = rowById(id);
  if (row >= 0)
