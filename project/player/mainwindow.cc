...
MainWindow::MainWindow(QWidget *parent, Qt::WindowFlags f)
  : Base(parent, f),
    settings_(Settings::globalInstance()),
    dragPos_(BAD_POS),
    lastOpenedPath_(INIT_OPEN_PATH)
{
  ...
  QString password = settings_->password();
...
#define MAKE_TOGGLE(_action, _styleid, _receiver, _slot) { \
  ...
  MAKE_TOGGLE(toggleTrendViewVisibleAct_, TRENDVIEW, trendView_, SLOT(setVisible(bool)))
...
void
MainWindow::submitAlias(const Alias &input)
{
  Alias a = input;
...
void
MainWindow::setToken(const QString &input)
{
...
void
MainWindow::submitText(const QString &text)
{
  if (!server_->isAuthorized() ||
      !tokenView_->hasToken() ||
...
void
MainWindow::chat(const QString &input)
{
  QString text = input.trimmed();
  if (text.isEmpty())
    return;

  showText(CORE_CMD_COLOR_BLUE " " + text);
  say(text, "blue");
...
bool
MainWindow::isPosNearOSDPlayer(const QPoint &pos) const
{
  return pos.x() < 6 || pos.x() > osdDock_->width() - 6 ||
         pos.y() < 6 || pos.y() > osdDock_->height() / 4 * 3 - 6;
}
...
MainWindow::mouseMoveEvent(QMouseEvent *event)
{
  // Prevent auto hide osd player.
  if (hub_->isFullScreenVideoMode())
    if (!event || isPosNearOSDPlayer(event->globalPos())) {
      if (!hasVisiblePlayer())
        osdPlayer_->show();
      else if (osdPlayer_->isVisible())
        osdPlayer_->resetAutoHideTimeout();
...
#undef ADD_RECENT
     ...
     {
       contextMenu_->addSeparator();
       toggleAnnotationVisibleAct_->setChecked(annotationView_->isPlaybackEnabled());
    ...
    if (hub_->isMediaTokenMode() && player_->hasMedia()) {
      ...
      // Subtitle menu
      {
        ...
        if (player_->hasSubtitles()) {
          toggleSubtitleVisibleAct_->setChecked(player_->isSubtitleVisible());
    ...
    // Toggles
    {
      ...
      if (commentView_->tokenId()) {
        toggleCommentViewVisibleAct_->setChecked(commentView_->isVisible());
        contextMenu_->addAction(toggleCommentViewVisibleAct_ );
      }

      toggleTrendViewVisibleAct_->setChecked(trendView_->isVisible());
      contextMenu_->addAction(toggleTrendViewVisibleAct_ );
    }
...
MainWindow::changeEvent(QEvent *event)
{
  if (event)
    ...
    case QEvent::WindowStateChange:
      switch (windowState()) {
      case Qt::WindowMaximized: hub_->setFullScreenVideoMode(); break;
      default: break;
      } break;
    default: break;
    }
...
void
MainWindow::login(const QString &userName, const QString &encryptedPassword)
{
  ...
  settings_->setPassword(encryptedPassword);
  server_->login(userName, encryptedPassword);
  if (server_->isConnected() && server_->isAuthorized()) {

    if (!server_->user().hasLanguage())
      server_->setUserLanguage(Traits::English);

    qint64 uid = server_->user().id();
    settings_->setUserId(uid);
...
MainWindow::openProcessPath(const QString &path)
{
  ...
  else if (QTH->isProcessAttached(pid))
...
void
MainWindow::setUserAnonymous(bool t)
{
  if (server_->isConnected() && server_->isAuthorized()) {
...
void
MainWindow::setUserLanguage(int lang)
{
  if (server_->isConnected() && server_->isAuthorized()) {
