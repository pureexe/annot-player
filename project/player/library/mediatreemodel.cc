// annotationitemmodel.cc
// 10/3/2011

#include "annotationitemmodel.h"
#include "tr.h"
#include "signalhub.h"
...
AnnotationItemModel::AnnotationItemModel(SignalHub *hub, QObject *parent)
  : Base(0, HD_Count, parent), hub_(hub)
{ resetHeaderData(); }

void
AnnotationItemModel::setHeaderData(QAbstractItemModel *model)
{
  Q_ASSERT(model);
...
int
AnnotationItemModel::rowById(qint64 id) const
{
...
qint64
AnnotationItemModel::indexId(const QModelIndex &input)
{
  if (!input.isValid())
    return 0;
  int row = input.row();
...
qint64
AnnotationItemModel::indexUserId(const QModelIndex &input)
{
  if (!input.isValid())
    return 0;
  int row = input.row();
...
QString
AnnotationItemModel::indexUserAlias(const QModelIndex &input)
{
  if (!input.isValid())
    return QString();
...
QString
AnnotationItemModel::indexText(const QModelIndex &input)
{
  if (!input.isValid())
    return QString();
...
QString
AnnotationItemModel::languageToString(int lang)
{
  switch(lang) {
...
QStringList
AnnotationItemModel::annotationFlagsToStringList(int flags)
{
  QStringList ret;
...
QString
AnnotationItemModel::annotationStatusToString(int status)
{
  switch (status) {
...
void
AnnotationItemModel::updateAnnotationTextWithId(const QString &text, qint64 id)
{
  int row = rowById(id);
  if (row >= 0) {
...
void
AnnotationItemModel::updateAnnotationUserIdWithId(qint64 userId, qint64 id)
{
  int row = rowById(id);
  if (row >= 0)
