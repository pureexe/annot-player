...
#include <Psapi.h>      // Paspi.dll
#include <ShellAPI.h>   // Shell32.dll
#include <ShlGuid.h>    // -
#include <ShlObj.h>     // -
#include <ShObjIdl.h>   // -
//#include <Shlwapi.h>
#include <TlHelp32.h>   // Kernel32.dll
//#include <strsafe.h>
...
#define QPATH_WINPATH(_qs)      QSTRING_LPCWSTR(QDir::toNativeSeparators((_qs)))
...
QtWin::setFocus(QWidget *w)
{
  ...
  bool succeed = ::SetFocus(detail::WId2HWND(w->winId()));
  w->setFocus();
  return succeed;
}
#endif // QT_WIDGETS_LIB
...
bool
QtWin::setFocus(WId wid)
{ return ::SetFocus(detail::WId2HWND(wid)); }

void
QtWin::repaintWindow(WId wid)
{ ::InvalidateRect(detail::WId2HWND(wid), NULL, TRUE); }

WId
QtWin::getChildWindow(WId wid)
{
  if (!wid)
    return 0;
  HWND child = ::GetWindow(detail::WId2HWND(wid), GW_CHILD);
  return child && ::IsWindow(child) ? detail::HWND2WId(child) : 0;
}

QList<WId>
QtWin::getChildWindows(WId wid)
{
  QList<WId> ret;
  WId h = wid;
  if (h)
...
WId
QtWin::getChildWindowAtPos(const QPoint &globalPos, WId parent)
{
  return detail::HWND2WId(
        ::ChildWindowFromPoint(
          detail::WId2HWND(parent),
          detail::QPoint2POINT(globalPos)));
}

QRect
QtWin::getWindowRect(WId wid)
{
  QRect ret;
  RECT rect;
  if (::GetWindowRect(detail::WId2HWND(wid), &rect))
    ret = detail::RECT2QRect(rect);
  return ret;
}

bool
QtWin::windowHasRect(WId wid)
{
  RECT rect;
  return ::GetWindowRect(detail::WId2HWND(wid), &rect) &&
...
bool
QtWin::isWindowValid(WId wid)
{ return ::IsWindow(detail::WId2HWND(wid)); }

bool
QtWin::isWindowVisible(WId wid)
{ return ::IsWindowVisible(detail::WId2HWND(wid)); }

bool
QtWin::isWindowMinimized(WId wid)
{ return ::IsIconic(detail::WId2HWND(wid)); }

bool
QtWin::setTopWindow(WId wid)
{ return ::BringWindowToTop(detail::WId2HWND(wid)); }

WId
QtWin::getTopWindow(WId wid)
{
  return detail::HWND2WId
      (::GetTopWindow(
         detail::WId2HWND(wid)));
...
WId
QtWin::getPreviousWindow(WId wid)
{
  return detail::HWND2WId(
        ::GetNextWindow(
          detail::WId2HWND(wid),
...
{
  HWND next = detail::WId2HWND(parent);
  while (next = ::GetNextWindow(next, GW_HWNDNEXT))
    if (next == detail::WId2HWND(target))
      return true;
  return false;
}
...
bool
QtWin::windowHasText(WId wid)
{
  wchar_t wsz[2];
  int count = ::GetWindowTextW(
        detail::WId2HWND(wid),
        wsz, sizeof(wsz)/sizeof(*wsz));
  return count && wsz[0];
}

QString
QtWin::getWindowText(WId wid, size_t size)
{
  ...
  if (size) {
    ...
    size = ::GetWindowTextW(detail::WId2HWND(wid), wsz, size);
    if (size)
      ret = QString::fromWCharArray(wsz, size);
    delete wsz;
  } else {
    wchar_t wsz[MAX_SIZE];
    size = ::GetWindowTextW(detail::WId2HWND(wid), wsz, sizeof(wsz)/sizeof(*wsz));
    if (size)
      ret = QString::fromWCharArray(wsz, size);
  }
  return ret;
...
DWORD
QtWin::getWindowThreadId(WId wid)
{ return ::GetWindowThreadProcessId(detail::WId2HWND(wid), 0); }

DWORD
QtWin::getWindowProcessId(WId wid)
{
  DWORD dwProcessId = 0;
  ::GetWindowThreadProcessId(detail::WId2HWND(wid), &dwProcessId);
  return dwProcessId;
}
...
namespace { // anonymous
  ...
  getWindowWithThreadId_p(HWND hwnd, LPARAM lparam)
  {
    ...
    Q_ASSERT(lp);
    if (QtWin::getWindowThreadId(detail::HWND2WId(hwnd)) == lp->dword) {
      lp->hwnd = hwnd;
  ...
  getVisibleWindowWithThreadId_p(HWND hwnd, LPARAM lparam)
  {
    ...
    if (::IsWindowVisible(hwnd) &&
        QtWin::getWindowThreadId(detail::HWND2WId(hwnd) ) == lp->dword) {
      lp->hwnd = hwnd;
      return false; // break enumeration
    } else
      return true; // continue enumeration
  ...
  getGoodWindowWithThreadId_p(HWND hwnd, LPARAM lparam)
  {
    ...
    Q_ASSERT(lp);
    if (QtWin::getWindowThreadId(detail::HWND2WId(hwnd)) == lp->dword &&
        QtWin::isGoodWindow(detail::HWND2WId(hwnd))) {
      lp->hwnd = hwnd;
      return false; // break enumeration
    } else
      return true; // continue enumeration
  ...
  getWindowWithProcessId_p(HWND hwnd, LPARAM lparam)
  {
    auto lp = reinterpret_cast<detail::PHWND_DWORD>(lparam);
    Q_ASSERT(lp);
    if (QtWin::getWindowProcessId(detail::HWND2WId(hwnd)) == lp->dword) {
      lp->hwnd = hwnd;
      return false; // break enumeration
    } else
      return true; // continue enumeration
  ...
  getVisibleWindowWithProcessId_p(HWND hwnd, LPARAM lparam)
  {
    ...
    Q_ASSERT(lp);
    if (::IsWindowVisible(hwnd) &&
        QtWin::getWindowProcessId(detail::HWND2WId(hwnd)) == lp->dword) {
      lp->hwnd = hwnd;
      return false; // break enumeration
    } else
      return true; // continue enumeration
...
QtWin::getWindowWithThreadId(DWORD threadId)
{
  ...
  return detail::HWND2WId(tuple.hwnd);
}
...
QString
QtWin::getFontsPath()
{ return RegIO::readValue(REG_HKCU_SHELLFOLDERS, "Fonts").toString(); }

QString
QtWin::getMusicPath()
{ return RegIO::readValue(REG_HKCU_SHELLFOLDERS, "My Music").toString(); }

QString
QtWin::getVideoPath()
{ return RegIO::readValue(REG_HKCU_SHELLFOLDERS, "My Video").toString(); }

QString
QtWin::getDocumentsPath()
{ return RegIO::readValue(REG_HKCU_SHELLFOLDERS, "Personal").toString(); }
...
// Requiring Ole32.dll.
QString
QtWin::resolveLink(const QString &lnkPath, WId wid)
{
  QString ret;
  if (lnkPath.isEmpty())
    return ret;
  ...
  if (SUCCEEDED(hres))  {
    ...
    if (SUCCEEDED(hres))  {
     ...
     if (SUCCEEDED(hres))  {
       // Resolve the link.
       if (wid)
         hres = psl->Resolve(detail::WId2HWND(wid), 0);
       if (SUCCEEDED(hres))  {
...
bool
QtWin::trashFile(const QString &fileName, bool confirm, WId wid)
{
...
QtWin::isFileExists(const QString &fileName)
{
  ...
  return ::PathFileExistsW(QPATH_WINPATH(fileName));
#else
...
{ return ::DeleteFileW(QPATH_WINPATH(fileName)); }
...
{ return ::GetFileAttributesW(QPATH_WINPATH(fileName)); }

bool
QtWin::setFileAttributes(const QString &fileName, uint flags)
{ return ::SetFileAttributesW(QPATH_WINPATH(fileName), flags); }
...
{ ::SHAddToRecentDocs(SHARD_PATHW, QPATH_WINPATH(fileName)); }

// - Cursor -

QtWin::CursorHandle
QtWin::setCursor(CursorHandle hcur)
{ return ::SetCursor(static_cast<HCURSOR>(hcur)); }
...
QtWin::CursorHandle
QtWin::loadCursorFromFile(const QString &fileName)
{ return ::LoadCursorFromFileW(QPATH_WINPATH(fileName)); }

QtWin::CursorHandle
QtWin::loadCursorFromFile(const wchar_t *fileName)
{ return ::LoadCursorFromFileW(fileName); }

QtWin::CursorHandle
QtWin::copyCursor(CursorHandle hcur)
{ return CopyCursor(static_cast<HCURSOR>(hcur)); } // CopyCursor is a global macro in Windows.h

bool
QtWin::destroyCursor(CursorHandle hcur)
{ return ::DestroyCursor(static_cast<HCURSOR>(hcur)); }

bool
QtWin::setSystemCursor(CursorHandle hcur, CursorShape shape)
{ return ::SetSystemCursor(static_cast<HCURSOR>(hcur), shape); }
