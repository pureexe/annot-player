...
WbSearchEngineFactory::create(int engine) const
{
  ...
  case WikiEn:    return new WbSearchEngine(WikiEn, WBSE_WIKI_EN, tr("Wikipedia"), WBRC_IMAGE_WIKI, QStringList("en"));
  ...
  case CPlusPlus: return new WbSearchEngine(Qt, WBSE_CPP, "C++", WBRC_IMAGE_CPP, QStringList("cpp") << "c++" << "cplusplus");
  case Bash:      return new WbSearchEngine(Qt, WBSE_BASH, "Bash", WBRC_IMAGE_SS64, QStringList("bash") << "sh" << "shell");
  case Cmd:       return new WbSearchEngine(Qt, WBSE_CMD, "Cmd", WBRC_IMAGE_SS64, QStringList("cmd") << "dos" << "nt");
  case Mac:       return new WbSearchEngine(Qt, WBSE_MAC, "Mac", WBRC_IMAGE_SS64, QStringList("osx"));
